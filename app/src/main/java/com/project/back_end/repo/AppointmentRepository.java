...
*/
    ...
    List<Appointment> findByPatient_IdAndStatusOrderByAppointmentTimeAsc(Long patientId, Integer status);
    ...
    // filterByDoctorNameAndPatientIdAndStatus:
    // Retrieves appointments based on a doctor’s name (using a LIKE query), patient’s ID, and a specific appointment status.
    @Query("SELECT a FROM Appointment a WHERE a.doctor.name LIKE %:doctorName% AND a.patient.id = :patientId AND a.status = :status")
    List<Appointment> filterByDoctorNameAndPatientIdAndStatus(@Param("doctorName") String doctorName, @Param("patientId") Long patientId, @Param("status") Integer status);

    // updateStatus:
    // Updates the status of a specific appointment based on its ID.
    // Requires @Modifying and @Transactional.
    // Uses a custom JPQL update query.
    @Modifying
    @Transactional
